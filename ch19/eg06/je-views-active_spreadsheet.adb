----------------------------------------------------------------------------
--
--   File:    je-views-active_spreadsheet.adb
--   Purpose: User interface package (body) for active spreadsheet
...
package body JE.Views.Active_Spreadsheet is
    use JE.Spreadsheets, JE.Spreadsheets.Active;

    function Next_Command (Sheet : access Sheet_Type) return Command_Type is
        Command   : Character;
        Available : Boolean;
    begin
        loop
    ...
    begin
        ...
        if Which = null then                            -- empty cell
            ...
            if Which.all in JE.Spreadsheets.String_Cell_Type'Class then
                Put ('"');                              -- string cell
            end if;
