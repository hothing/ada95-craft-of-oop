...
--   Purpose: User interface package (body) for meetings diary program
...
with Ada.Text_IO, Ada.Integer_Text_IO, JE.Times, JE.Appointments.Meetings;
...
package body Diary_View is
    ...
    procedure Add_Appointment (Diary : in out JE.Diaries.Diary_Type) is
        ...
        Meeting   : JE.Appointments.Meetings.Meeting_Type;
    begin
        Put ("Enter date:  ");
        Get (Day);
        Get (Separator);
        ...
        Put ("Room number: ");
        Get (Room);
        JE.Appointments.Meetings.Meeting
                  ( JE.Times.Time (Year, Month, Day, Hour, Minute),
                    Details(1..Length),
                    Room,
                    Meeting );
        JE.Diaries.Add ( Diary, Meeting );
    exception
        when Data_Error | Constraint_Error | JE.Times.Time_Error =>
            Put_Line ("Invalid input.");
    end Add_Appointment;

    procedure Put (Appt : in JE.Appointments.Meetings.Meeting_Type) is
        package Month_IO is new Enumeration_IO (JE.Times.Month_Type);
        use Month_IO, JE.Times, JE.Appointments.Meetings;
    begin
    ...
    procedure List_Appointments (Diary : in JE.Diaries.Diary_Type) is
        use JE.Diaries, JE.Appointments.Meetings;
    begin
